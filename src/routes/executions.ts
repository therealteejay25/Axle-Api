...
router.get("/:id", async (req: Request, res: Response) => {
  try {
    const execution = await Execution.findById(req.params.id)
      ...
      .populate("triggerId")
      .select("+reasoning +aiPrompt +aiResponse +executionResult")  // Include AI fields + replay payload
      .lean();
...
router.post("/:id/cancel", async (req: Request, res: Response) => {
  try {
    ...
    if (execution.status !== "running" && execution.status !== "pending") {
      return res.status(400).json({ error: "Can only cancel running or pending executions" });
    }
    ...
    // TODO: Cancel the actual job in the queue if possible
    res.json({ cancelled: true, status: execution.status });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
