export const AXLE_PROMPT = `You are Axle, an AI-powered workflow automation platform that replaces manual ops work with smart agents. You connect to everyday tools, listen for triggers, and execute tasks automatically—without needing Zapier-style configuration hell or dev-level scripting.
...
export interface PromptContext {
  ...
  appData: {
    ...
    statistics?: {
      ...
      recentRuns: Array<{ name: string; lastRunAt?: string }>;
    };
    toolCategories?: Record<string, string[]>;
  };
}

// Extended context for comprehensive agent intelligence
...
export const buildPromptWithContext = (context: PromptContext | ExtendedPromptContext): string => {
  const basePrompt = AXLE_PROMPT;
...
No agents configured yet.`;
  ...
  const integrationsSection =
    context.integrations && context.integrations.length > 0
      ? `
## Your Integrations (${context.integrations.length})
${context.integrations
  .map(
    (integration) => `
- **${integration.name}**: Connected ${integration.connectedAt}${
      integration.scope ? ` (Scope: ${integration.scope.join(", ")})` : ""
    }`
  )
  .join("\n")}`
      : `
...
No integrations connected yet.`;
  ...
  const stats = (context.appData as any).statistics;
  const toolCats = (context.appData as any).toolCategories || {};
...
{ "type": "tool", "target": "axle_create_agent", "args": { "ownerId": "${
    context.user.id
  }", "name": "GitHub Job Scout", "description": "Finds job opportunities based on GitHub profile", "systemPrompt": "You analyze the user's GitHub profile including repositories, skills, and experience level to identify and recommend relevant job opportunities...", "tools": ["searchGitHub", "list_pull_requests", "http_requester"], "integrations": ["github"] } }
