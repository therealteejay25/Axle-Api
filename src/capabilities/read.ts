...
export const readActions: Record<string, ActionDefinition> = {
    ...
    executor: async (inputs: ActionInputs, context: ExecutionContext) => {
      ...
      if (inputs.extractKeyPoints && result.content) {
        ...
        keyPoints = paragraphs
          ...
          .filter((p: string) => p.length > 20 && p.length < 200);
      }
  ...
  read_code: {
    ...
    executor: async (inputs: ActionInputs, context: ExecutionContext) => {
      ...
      switch (inputs.type) {
        case 'readme':
          data = await githubActions.github_get_repo_readme(
            { owner, repo },
            integration
          );
          break;
        ...
        case 'info':
          data = await githubActions.github_get_repo(
            { owner, repo },
            integration
          );
          break;
        ...
        case 'issues':
          data = await githubActions.github_list_issues(
            { owner, repo, state: 'open', perPage: inputs.limit || 10 },
            integration
          );
          break;
          
        case 'prs':
          data = await githubActions.github_list_prs(
            { owner, repo, state: 'open', perPage: inputs.limit || 10 },
            integration
          );
          break;
