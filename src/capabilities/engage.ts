...
export const engageActions: Record<string, ActionDefinition> = {
  
  // ==================== LIKE ITEM ====================
  engage_like: {
    actionId: 'engage_like',
    capability: Capability.ENGAGE,
    ...
    executor: async (inputs: ActionInputs, context: ExecutionContext) => {
      if (inputs.platform === 'x') {
        ...
        if (!integration) throw new Error('Twitter/X integration not connected');
