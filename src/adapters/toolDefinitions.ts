import { ToolDefinition, ToolCapability } from './types';
...
export const toolDefinitions: Record<string, ToolDefinition> = {
  ...
  'github_list_repos': {
    ...
    capability: ToolCapability.READ_CONTENT,
    provider: 'github',
  ...
  'github_create_issue': {
    ...
    capability: ToolCapability.WRITE_CONTENT,
    provider: 'github',
  ...
  'github_merge_pr': {
    ...
    capability: ToolCapability.CODE_MANAGEMENT,
    provider: 'github',
...
export const getToolDefinition = (toolName: string): ToolDefinition | null => {
  return toolDefinitions[toolName] || null;
};

// Get all tools for a specific capability
export const getToolsByCapability = (
  capability: ToolCapability,
  connectedIntegrations: string[]
): ToolDefinition[] => {
  return Object.values(toolDefinitions).filter(tool => {
    // Match capability
    if (tool.capability !== capability) return false;
...
export const getAvailableToolDefinitions = (
  connectedIntegrations: string[]
): ToolDefinition[] => {
  return Object.values(toolDefinitions).filter(tool => {
