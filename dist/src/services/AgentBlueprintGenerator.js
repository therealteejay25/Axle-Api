...
exports.AgentBlueprintGenerator = void 0;
const aiCaller_1 = require("../worker/aiCaller");
const logger_1 = require("./logger");
class AgentBlueprintGenerator {
    static async generateFromPrompt(prompt) {
        logger_1.logger.info("Generating agent blueprint from prompt", { promptLength: prompt.length });
        const systemPrompt = `
    ...
    `;
        try {
            const response = await (0, aiCaller_1.callAI)(systemPrompt, "gemini-1.5-pro-002", 0.3);
            ...
            logger_1.logger.error("Failed to generate agent blueprint", { error: error.message });
            throw new Error(`Blueprint generation failed: ${error.message}`);
        }
    }
}
exports.AgentBlueprintGenerator = AgentBlueprintGenerator;
