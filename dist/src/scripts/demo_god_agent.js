...
async function demoGodAgent() {
    ...
    // 1. Setup a test user
    const user = await User_1.User.findOne() || await User_1.User.create({ email: "god_agent_test@axle.ai" });
    console.log(`Using user: ${user.email} (${user._id})`);
    // 2. Scenario: "What are my agents doing?"
    console.log("\n--- Scenario 1: Data Query ---");
    const response1 = await axleChatbot_1.AxleChatbot.processMessage(user._id.toString(), "What agents do I have and what was the last one that ran?");
    console.log("God Agent Response:", response1.response);
    // 3. Scenario: "Pause the Slack bot"
    console.log("\n--- Scenario 2: Agent Management ---");
    const response2 = await axleChatbot_1.AxleChatbot.processMessage(user._id.toString(), "I think my Slack bot is being too noisy. Can you pause it for me?");
    ...
    const response3 = await axleChatbot_1.AxleChatbot.processMessage(user._id.toString(), "Send a message to the #general channel saying 'The God Agent is now online!'");
    ...
    const response4 = await axleChatbot_1.AxleChatbot.processMessage(user._id.toString(), "Delete my 'old-v1-abandoned' repository on GitHub.");
