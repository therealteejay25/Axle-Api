...
exports.getAdkTools = getAdkTools;
const discover_1 = require("./discover");
const read_1 = require("./read");
const write_1 = require("./write");
const notify_1 = require("./notify");
const engage_1 = require("./engage");
...
// Combine all actions
const allActions = {
    ...discover_1.discoverActions,
    ...read_1.readActions,
    ...write_1.writeActions,
    ...notify_1.notifyActions,
    ...engage_1.engageActions,
    ...organize_1.organizeActions
};
...
function checkRateLimit(action) {
    if (!action.constraints.rateLimit)
        return;
    ...
    if (usage.currentHour >= limits.maxPerHour) {
        throw new Error(`Rate limit exceeded for ${action.actionId}: ${limits.maxPerHour}/hour. ` +
            `Resets at ${usage.resetHour.toISOString()}`);
    }
    if (usage.currentDay >= limits.maxPerDay) {
        throw new Error(`Rate limit exceeded for ${action.actionId}: ${limits.maxPerDay}/day. ` +
...
function validateInputs(action, inputs) {
    ...
    if (action.actionId === 'write_post') {
        const rawPlatform = inputs.platform;
        if (typeof rawPlatform === 'string') {
            ...
            if (normalized === 'twitter') {
                ...
                inputs.platform = normalized;
            }
        }
    }
    if (action.actionId === 'write_document') {
        const hasTitle = inputs.title !== undefined && inputs.title !== null && String(inputs.title).trim() !== '';
        if (!hasTitle) {
            ...
            inputs.title = safeDerived || `Untitled document (${new Date().toISOString().slice(0, 10)})`;
        }
    }
    for (const [fieldName, fieldDef] of Object.entries(schema)) {
        ...
        if (actualType !== fieldDef.type) {
            throw new Error(`Invalid type for ${fieldName}: expected ${fieldDef.type}, got ${actualType}`);
        }
        // Enum validation
        if (fieldDef.enum && !fieldDef.enum.includes(value)) {
            throw new Error(`Invalid value for ${fieldName}: must be one of ${fieldDef.enum.join(', ')}`);
        }
        // String/number validation
        if (fieldDef.validation) {
            if (fieldDef.type === 'string' && typeof value === 'string') {
                if (fieldDef.validation.min && value.length < fieldDef.validation.min) {
                    throw new Error(`${fieldName} must be at least ${fieldDef.validation.min} characters`);
                }
                if (fieldDef.validation.max && value.length > fieldDef.validation.max) {
                    throw new Error(`${fieldName} must be at most ${fieldDef.validation.max} characters`);
                }
                if (fieldDef.validation.pattern) {
                    const regex = new RegExp(fieldDef.validation.pattern);
                    if (!regex.test(value)) {
                        throw new Error(`${fieldName} does not match required pattern: ${fieldDef.validation.pattern}`);
                    }
                }
            }
            if (fieldDef.type === 'number' && typeof value === 'number') {
                if (fieldDef.validation.min !== undefined && value < fieldDef.validation.min) {
                    throw new Error(`${fieldName} must be at least ${fieldDef.validation.min}`);
                }
                if (fieldDef.validation.max !== undefined && value > fieldDef.validation.max) {
                    throw new Error(`${fieldName} must be at most ${fieldDef.validation.max}`);
                }
            }
        }
    }
...
// VERIFICATION
// ============================================
async function verifyResult(action, result, context) {
    ...
    if (action.verification.method === 'read_back') {
        ...
        for (const [field, type] of Object.entries(expected)) {
            if (!(field in result)) {
                logger_1.logger.warn(`Verification failed: missing field ${field}`, {
            ...
            if (actualType !== type && !(type === 'array' && Array.isArray(result[field]))) {
                logger_1.logger.warn(`Verification failed: ${field} type mismatch`, {
                    action: action.actionId,
                    expected: type,
                    actual: actualType
                });
...
async function executeAction(actionId, inputs, context) {
    ...
    logger_1.logger.info('Executing capability action', {
        actionId,
...
// ============================================
function getAvailableActions(connectedIntegrations) {
    return Object.values(allActions).filter(action => {
        ...
        return action.metadata.requiresIntegration.some(required => connectedIntegrations.includes(required));
    });
}
// ============================================
// GET ACTION BY ID
...
function validateActionParams(actionId, params) {
    const action = allActions[actionId];
    if (!action) {
...
const create_issue_1 = require("./plugins/github/create_issue");
/**
 * Adapter to allow legacy ActionDefinitions to function as ADK Tools
 */
class LegacyBridgeTool extends BaseTool_1.BaseTool {
    action;
    constructor(action) {
        super();
        this.action = action;
 ...
 */
function getAdkTools(integrations, settings) {
    const tools = [];
    ...
    const plugins = [
        new create_issue_1.CreateIssueTool()
    ];
