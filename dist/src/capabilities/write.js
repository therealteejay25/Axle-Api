...
// Create new content
// ============================================
exports.writeActions = {
    // ==================== WRITE CODE ISSUE ====================
    write_code_issue: {
        actionId: 'write_code_issue',
        capability: types_1.Capability.WRITE,
    ...
    write_message: {
        ...
        executor: async (inputs, context) => {
            if (inputs.platform === 'slack') {
                ...
                const result = await slackActions.slack_send_message({ channel: inputs.to, text: inputs.message, threadTs: inputs.thread }, integration);
            ...
            if (inputs.platform === 'email') {
                ...
                const result = await googleActions.google_gmail_send_email({ to: inputs.to, subject: inputs.subject || 'Message from Agent', body: inputs.message }, integration);
        ...
        executor: async (inputs, context) => {
            ...
            if (inputs.platform === 'x') {
                ...
                if (!inputs.mediaUrl) {
                    ...
                    if (!decoded)
                        throw new Error('Post content is required for X');
            ...
            if (inputs.platform === 'instagram') {
                ...
                if (!integration)
                    throw new Error('Instagram integration not connected');
                if (!inputs.mediaUrl)
                    throw new Error('Image URL required for Instagram post');
    ...
    write_comment: {
        ...
        executor: async (inputs, context) => {
            if (inputs.platform === 'github') {
                ...
                const result = await githubActions.github_comment_issue({ owner, repo, issueNumber: parseInt(issueNumber), body: inputs.comment }, integration);
            ...
            if (inputs.platform === 'slack') {
                ...
                const result = await slackActions.slack_send_message({ channel, text: inputs.comment, threadTs }, integration);
...
exports.default = exports.writeActions;
