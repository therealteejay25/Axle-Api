...
exports.buildSystemPrompt = exports.buildIterativeContext = exports.buildContext = void 0;
const buildContext = (loaded, triggerType, triggerPayload, previousExecutions) => {
    return {
        agent: {
...
// ============================================
const buildIterativeContext = (loaded, triggerType, triggerPayload, iteration, maxIterations, actionHistory, observations, iterativeMemory, // Changed to structured memory
previousExecutions) => {
    const baseContext = (0, exports.buildContext)(loaded, triggerType, triggerPayload, previousExecutions);
    return {
        ...baseContext,
        iteration,
        maxIterations,
...
exports.buildIterativeContext = buildIterativeContext;
// Build the system prompt for the AI
const buildSystemPrompt = (loaded, context) => {
    console.log("=== BUILD SYSTEM PROMPT START ===");
    console.log("buildSystemPrompt called with context:", JSON.stringify(context.trigger, null, 2));
    // SIMPLIFIED UX: Use instructions if present, otherwise fall back to brain.systemPrompt
    const baseInstructions = loaded.agent.instructions ||
    ...
    console.log("Context trigger payload:", JSON.stringify(context.trigger.payload, null, 2));
    console.log("Task from payload:", taskFromPayload);
    console.log("Context iteration:", context.iteration, "typeof:", typeof context.iteration);
...
`
            : "";
        const integrationsInfo = context.availableIntegrations.length > 0
            ? `
...
`
        ...
        const prompt = `You are ${loaded.agent.name || "an AI agent"}. ${loaded.agent.description || ""}
...
${context.actionHistory
                .map((a, i) => `  ${i + 1}. ${a.type} - ${a.error ? "FAILED: " + a.error : "SUCCESS"}`)
                .join("\n")}
`
            : ""}
...
${context.iterativeMemory
                .map((m, i) => `  ${i + 1}. [${m.type}] ${m.source} @ ${m.timestamp}: ${JSON.stringify(m.payload)}`)
                .join("\n")}
`
            : ""}
...
${context.iteration && context.iteration > 1
            ? `
- Recovery attempts: ${context.recoveryAttempts || 0}/${context.maxRecoveryAttempts || 3}
${context.adjustmentsMade &&
                context.adjustmentsMade.length > 0
                ? `- Adjustments made: ${context.adjustmentsMade.join(", ")}`
                : ""}
${context.lastError
