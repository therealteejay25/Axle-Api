...
router.get("/providers", auth_1.authMiddleware, async (_req, res) => {
    res.json({
        providers: [
...
router.get("/events", auth_1.authMiddleware, async (_req, res) => {
    res.json({
        events: [
            // GitHub (5)
            {
...
router.get("/", auth_1.authMiddleware, async (req, res) => {
    try {
        const agentIds = (await Agent_1.Agent.find({ ownerId: req.user.id }).select("_id").lean()).map(a => a._id);
        const triggers = await Trigger_1.Trigger.find({
            ...
            .lean();
        const origin = env_1.env.PUBLIC_API_ORIGIN || "";
        const baseFromEnv = origin ? origin.replace(/\/$/, "") : "";
        const baseFromReq = `${req.protocol}://${req.get("host")}`;
        const webhooks = triggers.map((t) => {
            const webhookPath = t?.config?.webhookPath;
            const source = t?.config?.source;
            const relativeUrl = webhookPath ? `/api/${env_1.env.API_VERSION || "v1"}/webhooks/${webhookPath}` : undefined;
