...
router.get("/", async (req, res) => {
    try {
        const integrations = await Integration_1.Integration.find({ userId: req.user.id }).lean();
        const healthChecks = integrations.map(int => {
            ...
            const missingScopes = expectedScopes.filter(scope => !int.scopes.some(s => s.includes(scope) || scope.includes(s)));
